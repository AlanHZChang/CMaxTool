/* imx258_ms3_lib.h
 *
 * Copyright (c) 2015 Qualcomm Technologies, Inc.
 * All Rights Reserved.
 * Confidential and Proprietary - Qualcomm Technologies, Inc.
 */

#ifndef __IMX258_MS3_LIB_H__
#define __IMX258_MS3_LIB_H__

#include "sensor_lib.h"
#include "eeprom_lib.h"
#include "pdaf_api.h"
#include "pdaf_camif_api.h"
#define SENSOR_MODEL "imx258_ms3"

/* IMX258 Regs */
#define IMX258_MS3_DIG_GAIN_GR_ADDR       0x020E
#define IMX258_MS3_DIG_GAIN_R_ADDR        0x0210
#define IMX258_MS3_DIG_GAIN_B_ADDR        0x0212
#define IMX258_MS3_DIG_GAIN_GB_ADDR       0x0214
#define IMX258_MS3_EXP_RATIO_ADDR         0x0222
#define IMX258_MS3_ABS_GAIN_R_WORD_ADDR   0x0B90
#define IMX258_MS3_ABS_GAIN_B_WORD_ADDR   0x0B92

/* IMX258 CONSTANTS */
#define IMX258_MS3_MAX_INTEGRATION_MARGIN   20

/* STATS DATA TYPE */
#define IMX258_MS3_CSI_PD_ISTATS            0x2F

#define IMX258_MS3_DATA_PEDESTAL            0x40 /* 10bit value */

#define IMX258_MS3_MIN_AGAIN_REG_VAL        0 /* 1.0x */
#define IMX258_MS3_MAX_AGAIN_REG_VAL        448 /* 8.0x */

#define IMX258_MS3_MIN_DGAIN_REG_VAL        256 /* 1.0x */
#define IMX258_MS3_MAX_DGAIN_REG_VAL        3072 /* 12.0x */ /* 4057 / 15.84765625x / 24dB*/

#define IMX258_MS3_MAX_DGAIN_DECIMATOR      256

/* IMX258 FORMULAS */
#define IMX258_MS3_MIN_AGAIN    (512 / (512 - IMX258_MS3_MIN_AGAIN_REG_VAL))
#define IMX258_MS3_MAX_AGAIN    (512 / (512 - IMX258_MS3_MAX_AGAIN_REG_VAL))

#define IMX258_MS3_MIN_DGAIN    (IMX258_MS3_MIN_DGAIN_REG_VAL / 256)
#define IMX258_MS3_MAX_DGAIN    (IMX258_MS3_MAX_DGAIN_REG_VAL / 256)

#define IMX258_MS3_MIN_GAIN     IMX258_MS3_MIN_AGAIN * IMX258_MS3_MIN_DGAIN
#define IMX258_MS3_MAX_GAIN     IMX258_MS3_MAX_AGAIN * IMX258_MS3_MAX_DGAIN

/* uncomment FLIP_MIRROR macro to
   enable flip and mirror in sensor readout
   change bayer pattern
   load pdaf flip and mirror header
*/
//#define FLIP_MIRROR
#define START_REG_ARRAY \
{ \
  {0x0100, 0x01, 0x00}, \
}

#define STOP_REG_ARRAY \
{ \
  {0x0100, 0x00, 0x00}, \
}

#define GROUPON_REG_ARRAY \
{ \
  {0x0104, 0x01, 0x00}, \
}

#define GROUPOFF_REG_ARRAY \
{ \
  {0x0104, 0x00, 0x00}, \
}

#define INIT0_REG_ARRAY \
{ \
  /* External Clock Settings */ \
  {0x0136, 0x18, 0x00}, \
  {0x0137, 0x00, 0x00}, \
  /* Global Settings */ \
  {0x3051, 0x00, 0x00}, \
  {0x6B11, 0xCF, 0x00}, \
  {0x7FF0, 0x08, 0x00}, \
  {0x7FF1, 0x0F, 0x00}, \
  {0x7FF2, 0x08, 0x00}, \
  {0x7FF3, 0x1B, 0x00}, \
  {0x7FF4, 0x23, 0x00}, \
  {0x7FF5, 0x60, 0x00}, \
  {0x7FF6, 0x00, 0x00}, \
  {0x7FF7, 0x01, 0x00}, \
  {0x7FF8, 0x00, 0x00}, \
  {0x7FF9, 0x78, 0x00}, \
  {0x7FFA, 0x01, 0x00}, \
  {0x7FFB, 0x00, 0x00}, \
  {0x7FFC, 0x00, 0x00}, \
  {0x7FFD, 0x00, 0x00}, \
  {0x7FFE, 0x00, 0x00}, \
  {0x7FFF, 0x03, 0x00}, \
  {0x7F76, 0x03, 0x00}, \
  {0x7F77, 0xFE, 0x00}, \
  {0x7FA8, 0x03, 0x00}, \
  {0x7FA9, 0xFE, 0x00}, \
  {0x7B24, 0x81, 0x00}, \
  {0x7B25, 0x01, 0x00}, \
  {0x6564, 0x07, 0x00}, \
  {0x6B0D, 0x41, 0x00}, \
  {0x653D, 0x04, 0x00}, \
  {0x6B05, 0x8C, 0x00}, \
  {0x6B06, 0xF9, 0x00}, \
  {0x6B08, 0x65, 0x00}, \
  {0x6B09, 0xFC, 0x00}, \
  {0x6B0A, 0xCF, 0x00}, \
  {0x6B0B, 0xD2, 0x00}, \
  {0x6700, 0x0E, 0x00}, \
  {0x6707, 0x0E, 0x00}, \
  {0x9104, 0x00, 0x00}, \
  {0x7421, 0x1C, 0x00}, \
  {0x7423, 0xD7, 0x00}, \
  {0x5F04, 0x00, 0x00}, \
  {0x5F05, 0xED, 0x00}, \
  /* Image Quality Adjusment Settings */ \
  /* Defect Pixel Correction settings */ \
  {0x94C7, 0xFF, 0x00}, \
  {0x94C8, 0xFF, 0x00}, \
  {0x94C9, 0xFF, 0x00}, \
  {0x95C7, 0xFF, 0x00}, \
  {0x95C8, 0xFF, 0x00}, \
  {0x95C9, 0xFF, 0x00}, \
  {0x94C4, 0x3F, 0x00}, \
  {0x94C5, 0x3F, 0x00}, \
  {0x94C6, 0x3F, 0x00}, \
  {0x95C4, 0x3F, 0x00}, \
  {0x95C5, 0x3F, 0x00}, \
  {0x95C6, 0x3F, 0x00}, \
  {0x94C1, 0x02, 0x00}, \
  {0x94C2, 0x02, 0x00}, \
  {0x94C3, 0x02, 0x00}, \
  {0x95C1, 0x02, 0x00}, \
  {0x95C2, 0x02, 0x00}, \
  {0x95C3, 0x02, 0x00}, \
  {0x94BE, 0x0C, 0x00}, \
  {0x94BF, 0x0C, 0x00}, \
  {0x94C0, 0x0C, 0x00}, \
  {0x95BE, 0x0C, 0x00}, \
  {0x95BF, 0x0C, 0x00}, \
  {0x95C0, 0x0C, 0x00}, \
  {0x94D0, 0x74, 0x00}, \
  {0x94D1, 0x74, 0x00}, \
  {0x94D2, 0x74, 0x00}, \
  {0x95D0, 0x74, 0x00}, \
  {0x95D1, 0x74, 0x00}, \
  {0x95D2, 0x74, 0x00}, \
  {0x94CD, 0x2E, 0x00}, \
  {0x94CE, 0x2E, 0x00}, \
  {0x94CF, 0x2E, 0x00}, \
  {0x95CD, 0x2E, 0x00}, \
  {0x95CE, 0x2E, 0x00}, \
  {0x95CF, 0x2E, 0x00}, \
  {0x94CA, 0x4C, 0x00}, \
  {0x94CB, 0x4C, 0x00}, \
  {0x94CC, 0x4C, 0x00}, \
  {0x95CA, 0x4C, 0x00}, \
  {0x95CB, 0x4C, 0x00}, \
  {0x95CC, 0x4C, 0x00}, \
  {0x900E, 0x32, 0x00}, \
  {0x94E2, 0xFF, 0x00}, \
  {0x94E3, 0xFF, 0x00}, \
  {0x94E4, 0xFF, 0x00}, \
  {0x95E2, 0xFF, 0x00}, \
  {0x95E3, 0xFF, 0x00}, \
  {0x95E4, 0xFF, 0x00}, \
  {0x94DF, 0x6E, 0x00}, \
  {0x94E0, 0x6E, 0x00}, \
  {0x94E1, 0x6E, 0x00}, \
  {0x95DF, 0x6E, 0x00}, \
  {0x95E0, 0x6E, 0x00}, \
  {0x95E1, 0x6E, 0x00}, \
  {0x7FCC, 0x01, 0x00}, \
  {0x7B78, 0x00, 0x00}, \
}

#ifndef FLIP_MIRROR
  #define FLIP_MIRROR_SETTING {{0x0101, 0x00, 0x00}}
#else
  #define FLIP_MIRROR_SETTING {{0x0101, 0x03, 0x00}}
#endif

#define RES0_REG_ARRAY \
{ \
  /* Mode A1: 4208x3120 Full 30fps */ \
  /* Output Format Settings */ \
  {0x0112, 0x0A, 0x00}, \
  {0x0113, 0x0A, 0x00}, \
  {0x0114, 0x03, 0x00}, \
  /* Clock Settings */ \
  {0x0301, 0x05, 0x00}, \
  {0x0303, 0x02, 0x00}, \
  {0x0305, 0x04, 0x00}, \
  {0x0306, 0x00, 0x00}, \
  {0x0307, 0xD8, 0x00}, \
  {0x0309, 0x0A, 0x00}, \
  {0x030B, 0x01, 0x00}, \
  {0x030D, 0x02, 0x00}, \
  {0x030E, 0x00, 0x00}, \
  {0x030F, 0xD8, 0x00}, \
  {0x0310, 0x00, 0x00}, \
  {0x0820, 0x14, 0x00}, \
  {0x0821, 0x40, 0x00}, \
  {0x0822, 0x00, 0x00}, \
  {0x0823, 0x00, 0x00}, \
  /* Line Length Settings */ \
  {0x0342, 0x14, 0x00}, \
  {0x0343, 0xE8, 0x00}, \
  /* Frame Length Settings */ \
  {0x0340, 0x0C, 0x00}, \
  {0x0341, 0x98, 0x00}, \
  /* ROI Settings */ \
  {0x0344, 0x00, 0x00}, \
  {0x0345, 0x00, 0x00}, \
  {0x0346, 0x00, 0x00}, \
  {0x0347, 0x00, 0x00}, \
  {0x0348, 0x10, 0x00}, \
  {0x0349, 0x6F, 0x00}, \
  {0x034A, 0x0C, 0x00}, \
  {0x034B, 0x2F, 0x00}, \
  /* Analog Image Size Settings */ \
  {0x0381, 0x01, 0x00}, \
  {0x0383, 0x01, 0x00}, \
  {0x0385, 0x01, 0x00}, \
  {0x0387, 0x01, 0x00}, \
  {0x0900, 0x00, 0x00}, \
  {0x0901, 0x11, 0x00}, \
  /* Digital Image Size Settings */ \
  {0x0401, 0x00, 0x00}, \
  {0x0404, 0x00, 0x00}, \
  {0x0405, 0x10, 0x00}, \
  {0x0408, 0x00, 0x00}, \
  {0x0409, 0x00, 0x00}, \
  {0x040A, 0x00, 0x00}, \
  {0x040B, 0x00, 0x00}, \
  {0x040C, 0x10, 0x00}, \
  {0x040D, 0x70, 0x00}, \
  {0x040E, 0x0C, 0x00}, \
  {0x040F, 0x30, 0x00}, \
  {0x3038, 0x00, 0x00}, \
  {0x303A, 0x00, 0x00}, \
  {0x303B, 0x10, 0x00}, \
  {0x300D, 0x00, 0x00}, \
  /* Output Size Settings */ \
  {0x034C, 0x10, 0x00}, \
  {0x034D, 0x70, 0x00}, \
  {0x034E, 0x0C, 0x00}, \
  {0x034F, 0x30, 0x00}, \
  /* Integration Time Settings */ \
  {0x0202, 0x0C, 0x00}, \
  {0x0203, 0x8E, 0x00}, \
  /* Gain Settings */ \
  {0x0204, 0x00, 0x00}, \
  {0x0205, 0x00, 0x00}, \
  {0x020E, 0x01, 0x00}, \
  {0x020F, 0x00, 0x00}, \
  /* Added Settings IQ */ \
  {0x94DC, 0x20, 0x00}, \
  {0x94DD, 0x20, 0x00}, \
  {0x94DE, 0x20, 0x00}, \
  {0x95DC, 0x20, 0x00}, \
  {0x95DD, 0x20, 0x00}, \
  {0x95DE, 0x20, 0x00}, \
  {0x7FB0, 0x00, 0x00}, \
  {0x9010, 0x3E, 0x00}, \
  /* Added Settings mode */ \
  {0x3030, 0x01, 0x00}, \
  {0x3032, 0x01, 0x00}, \
  {0x0220, 0x00, 0x00}, \
  /* MIPI Global Timing Settings*/ \
  {0x080A, 0x00, 0x00}, \
  {0x080B, 0x97, 0x00}, \
  {0x080C, 0x00, 0x00}, \
  {0x080D, 0x5F, 0x00}, \
  {0x080E, 0x00, 0x00}, \
  {0x080F, 0x97, 0x00}, \
  {0x0810, 0x00, 0x00}, \
  {0x0811, 0x57, 0x00}, \
  {0x0812, 0x00, 0x00}, \
  {0x0813, 0x57, 0x00}, \
  {0x0814, 0x00, 0x00}, \
  {0x0815, 0x5F, 0x00}, \
  {0x0816, 0x01, 0x00}, \
  {0x0817, 0x67, 0x00}, \
  {0x0818, 0x00, 0x00}, \
  {0x0819, 0x47, 0x00}, \
}

#define RES1_REG_ARRAY \
{ \
  /* Mode: 4208x2368 4k 30 fps */ \
  /* Output Format Settings */ \
  {0x0112, 0x0A, 0x00}, \
  {0x0113, 0x0A, 0x00}, \
  {0x0114, 0x03, 0x00}, \
  /* Clock Settings */ \
  {0x0301, 0x05, 0x00}, \
  {0x0303, 0x02, 0x00}, \
  {0x0305, 0x04, 0x00}, \
  {0x0306, 0x00, 0x00}, \
  {0x0307, 0xA1, 0x00}, \
  {0x0309, 0x0A, 0x00}, \
  {0x030B, 0x01, 0x00}, \
  {0x030D, 0x02, 0x00}, \
  {0x030E, 0x00, 0x00}, \
  {0x030F, 0xD8, 0x00}, \
  {0x0310, 0x00, 0x00}, \
  {0x0820, 0x0F, 0x00}, \
  {0x0821, 0x18, 0x00}, \
  {0x0822, 0x00, 0x00}, \
  {0x0823, 0x00, 0x00}, \
  /* Line Length Settings */ \
  {0x0342, 0x14, 0x00}, \
  {0x0343, 0xE8, 0x00}, \
  /* Frame Length Settings */ \
  {0x0340, 0x09, 0x00}, \
  {0x0341, 0x60, 0x00}, \
  /* ROI Settings */ \
  {0x0344, 0x00, 0x00}, \
  {0x0345, 0x00, 0x00}, \
  {0x0346, 0x01, 0x00}, \
  {0x0347, 0x70, 0x00}, \
  {0x0348, 0x10, 0x00}, \
  {0x0349, 0x6F, 0x00}, \
  {0x034A, 0x0A, 0x00}, \
  {0x034B, 0xAF, 0x00}, \
  /* Analog Image Size Settings */ \
  {0x0381, 0x01, 0x00}, \
  {0x0383, 0x01, 0x00}, \
  {0x0385, 0x01, 0x00}, \
  {0x0387, 0x01, 0x00}, \
  {0x0900, 0x00, 0x00}, \
  {0x0901, 0x11, 0x00}, \
  /* Digital Image Size Settings */ \
  {0x0401, 0x00, 0x00}, \
  {0x0404, 0x00, 0x00}, \
  {0x0405, 0x10, 0x00}, \
  {0x0408, 0x00, 0x00}, \
  {0x0409, 0x00, 0x00}, \
  {0x040A, 0x00, 0x00}, \
  {0x040B, 0x00, 0x00}, \
  {0x040C, 0x10, 0x00}, \
  {0x040D, 0x70, 0x00}, \
  {0x040E, 0x09, 0x00}, \
  {0x040F, 0x40, 0x00}, \
  {0x3038, 0x00, 0x00}, \
  {0x303A, 0x00, 0x00}, \
  {0x303B, 0x10, 0x00}, \
  {0x300D, 0x00, 0x00}, \
  /* Output Size Settings */ \
  {0x034C, 0x10, 0x00}, \
  {0x034D, 0x70, 0x00}, \
  {0x034E, 0x09, 0x00}, \
  {0x034F, 0x40, 0x00}, \
  /* Integration Time Settings */ \
  {0x0202, 0x09, 0x00}, \
  {0x0203, 0x56, 0x00}, \
  /* Gain Settings */\
  {0x0204, 0x00, 0x00}, \
  {0x0205, 0x00, 0x00}, \
  {0x020E, 0x01, 0x00}, \
  {0x020F, 0x00, 0x00}, \
  /*Added Setting AF*/ \
  {0x7BCD, 0x00, 0x00}, \
  /* Added Settings IQ */ \
  {0x94DC, 0x20, 0x00}, \
  {0x94DD, 0x20, 0x00}, \
  {0x94DE, 0x20, 0x00}, \
  {0x95DC, 0x20, 0x00}, \
  {0x95DD, 0x20, 0x00}, \
  {0x95DE, 0x20, 0x00}, \
  {0x7FB0, 0x00, 0x00}, \
  {0x9010, 0x3E, 0x00}, \
  {0x9419, 0x50, 0x00}, \
  {0x941B, 0x50, 0x00}, \
  {0x9519, 0x50, 0x00}, \
  {0x951B, 0x50, 0x00}, \
  /* Added Settings mode */ \
  {0x3030, 0x01, 0x00}, \
  {0x3032, 0x01, 0x00}, \
  {0x0220, 0x00, 0x00}, \
  /* MIPI Global Timing Settings*/ \
  {0x080A, 0x00, 0x00}, \
  {0x080B, 0x97, 0x00}, \
  {0x080C, 0x00, 0x00}, \
  {0x080D, 0x5F, 0x00}, \
  {0x080E, 0x00, 0x00}, \
  {0x080F, 0x97, 0x00}, \
  {0x0810, 0x00, 0x00}, \
  {0x0811, 0x57, 0x00}, \
  {0x0812, 0x00, 0x00}, \
  {0x0813, 0x57, 0x00}, \
  {0x0814, 0x00, 0x00}, \
  {0x0815, 0x5F, 0x00}, \
  {0x0816, 0x01, 0x00}, \
  {0x0817, 0x67, 0x00}, \
  {0x0818, 0x00, 0x00}, \
  {0x0819, 0x47, 0x00}, \
}

#define RES2_REG_ARRAY \
{ \
  /* Mode: 2100x1560 2x2 binning 30 fps */ \
  /* Output Format Settings */ \
  {0x0112, 0x0A, 0x00}, \
  {0x0113, 0x0A, 0x00}, \
  {0x0114, 0x03, 0x00}, \
  /* Clock Settings */ \
  {0x0301, 0x05, 0x00}, \
  {0x0303, 0x02, 0x00}, \
  {0x0305, 0x04, 0x00}, \
  {0x0306, 0x00, 0x00}, \
  {0x0307, 0xD8, 0x00}, \
  {0x0309, 0x0A, 0x00}, \
  {0x030B, 0x01, 0x00}, \
  {0x030D, 0x02, 0x00}, \
  {0x030E, 0x00, 0x00}, \
  {0x030F, 0xD8, 0x00}, \
  {0x0310, 0x00, 0x00}, \
  {0x0820, 0x14, 0x00}, \
  {0x0821, 0x40, 0x00}, \
  {0x0822, 0x00, 0x00}, \
  {0x0823, 0x00, 0x00}, \
  /* Line Length Settings */ \
  {0x0342, 0x14, 0x00}, \
  {0x0343, 0xE8, 0x00}, \
  /* Frame Length Settings */ \
  {0x0340, 0x06, 0x00}, \
  {0x0341, 0x38, 0x00}, \
  /* ROI Settings */ \
  {0x0344, 0x00, 0x00}, \
  {0x0345, 0x00, 0x00}, \
  {0x0346, 0x00, 0x00}, \
  {0x0347, 0x00, 0x00}, \
  {0x0348, 0x10, 0x00}, \
  {0x0349, 0x6F, 0x00}, \
  {0x034A, 0x0C, 0x00}, \
  {0x034B, 0x2F, 0x00}, \
  /* Analog Image Size Settings */ \
  {0x0381, 0x01, 0x00}, \
  {0x0383, 0x01, 0x00}, \
  {0x0385, 0x01, 0x00}, \
  {0x0387, 0x01, 0x00}, \
  {0x0900, 0x01, 0x00}, \
  {0x0901, 0x12, 0x00}, \
  /* Digital Image Size Settings */ \
  {0x0401, 0x01, 0x00}, \
  {0x0404, 0x00, 0x00}, \
  {0x0405, 0x20, 0x00}, \
  {0x0408, 0x00, 0x00}, \
  {0x0409, 0x00, 0x00}, \
  {0x040A, 0x00, 0x00}, \
  {0x040B, 0x00, 0x00}, \
  {0x040C, 0x10, 0x00}, \
  {0x040D, 0x70, 0x00}, \
  {0x040E, 0x06, 0x00}, \
  {0x040F, 0x18, 0x00}, \
  {0x3038, 0x00, 0x00}, \
  {0x303A, 0x00, 0x00}, \
  {0x303B, 0x10, 0x00}, \
  {0x300D, 0x00, 0x00}, \
  /* Output Size Settings */ \
  {0x034C, 0x08, 0x00}, \
  {0x034D, 0x34, 0x00}, \
  {0x034E, 0x06, 0x00}, \
  {0x034F, 0x18, 0x00}, \
  /* Integration Time Settings */ \
  {0x0202, 0x06, 0x00}, \
  {0x0203, 0x2E, 0x00}, \
  /* Gain Settings */ \
  {0x0204, 0x00, 0x00}, \
  {0x0205, 0x00, 0x00}, \
  {0x020E, 0x01, 0x00}, \
  {0x020F, 0x00, 0x00}, \
  /* Added Settings IQ */ \
  {0x94DC, 0x20, 0x00}, \
  {0x94DD, 0x20, 0x00}, \
  {0x94DE, 0x20, 0x00}, \
  {0x95DC, 0x20, 0x00}, \
  {0x95DD, 0x20, 0x00}, \
  {0x95DE, 0x20, 0x00}, \
  {0x7FB0, 0x00, 0x00}, \
  {0x9010, 0x3E, 0x00}, \
  /* Added Settings mode */ \
  {0x3030, 0x01, 0x00}, \
  {0x3032, 0x01, 0x00}, \
  {0x0220, 0x00, 0x00}, \
  /* MIPI Global Timing Settings*/ \
  {0x080A, 0x00, 0x00}, \
  {0x080B, 0x97, 0x00}, \
  {0x080C, 0x00, 0x00}, \
  {0x080D, 0x5F, 0x00}, \
  {0x080E, 0x00, 0x00}, \
  {0x080F, 0x97, 0x00}, \
  {0x0810, 0x00, 0x00}, \
  {0x0811, 0x57, 0x00}, \
  {0x0812, 0x00, 0x00}, \
  {0x0813, 0x57, 0x00}, \
  {0x0814, 0x00, 0x00}, \
  {0x0815, 0x5F, 0x00}, \
  {0x0816, 0x01, 0x00}, \
  {0x0817, 0x67, 0x00}, \
  {0x0818, 0x00, 0x00}, \
  {0x0819, 0x47, 0x00}, \
}

#define RES3_REG_ARRAY \
{ \
  /* Mode: 2100x1176 60 fps */ \
  /* Output Format Settings */ \
  {0x0112, 0x0A, 0x00}, \
  {0x0113, 0x0A, 0x00}, \
  {0x0114, 0x03, 0x00}, \
  /* Clock Settings */ \
  {0x0301, 0x05, 0x00}, \
  {0x0303, 0x02, 0x00}, \
  {0x0305, 0x04, 0x00}, \
  {0x0306, 0x00, 0x00}, \
  {0x0307, 0xD8, 0x00}, \
  {0x0309, 0x0A, 0x00}, \
  {0x030B, 0x01, 0x00}, \
  {0x030D, 0x02, 0x00}, \
  {0x030E, 0x00, 0x00}, \
  {0x030F, 0xD8, 0x00}, \
  {0x0310, 0x00, 0x00}, \
  {0x0820, 0x14, 0x00}, \
  {0x0821, 0x40, 0x00}, \
  {0x0822, 0x00, 0x00}, \
  {0x0823, 0x00, 0x00}, \
  /* Line Length Settings */ \
  {0x0342, 0x14, 0x00}, \
  {0x0343, 0xE8, 0x00}, \
  /* Frame Length Settings */ \
  {0x0340, 0x06, 0x00}, \
  {0x0341, 0x4C, 0x00}, \
  /* ROI Settings */ \
  {0x0344, 0x00, 0x00}, \
  {0x0345, 0x00, 0x00}, \
  {0x0346, 0x01, 0x00}, \
  {0x0347, 0x80, 0x00}, \
  {0x0348, 0x10, 0x00}, \
  {0x0349, 0x6F, 0x00}, \
  {0x034A, 0x0A, 0x00}, \
  {0x034B, 0xAF, 0x00}, \
  /* Analog Image Size Settings */ \
  {0x0381, 0x01, 0x00}, \
  {0x0383, 0x01, 0x00}, \
  {0x0385, 0x01, 0x00}, \
  {0x0387, 0x01, 0x00}, \
  {0x0900, 0x01, 0x00}, \
  {0x0901, 0x12, 0x00}, \
  /* Digital Image Size Settings */ \
  {0x0401, 0x01, 0x00}, \
  {0x0404, 0x00, 0x00}, \
  {0x0405, 0x20, 0x00}, \
  {0x0408, 0x00, 0x00}, \
  {0x0409, 0x00, 0x00}, \
  {0x040A, 0x00, 0x00}, \
  {0x040B, 0x00, 0x00}, \
  {0x040C, 0x10, 0x00}, \
  {0x040D, 0x70, 0x00}, \
  {0x040E, 0x04, 0x00}, \
  {0x040F, 0x98, 0x00}, \
  {0x3038, 0x00, 0x00}, \
  {0x303A, 0x00, 0x00}, \
  {0x303B, 0x10, 0x00}, \
  {0x300D, 0x00, 0x00}, \
  /* Output Size Settings */ \
  {0x034C, 0x08, 0x00}, \
  {0x034D, 0x34, 0x00}, \
  {0x034E, 0x04, 0x00}, \
  {0x034F, 0x98, 0x00}, \
  /* Integration Time Settings */ \
  {0x0202, 0x06, 0x00}, \
  {0x0203, 0x42, 0x00}, \
  /* Gain Settings */ \
  {0x0204, 0x00, 0x00}, \
  {0x0205, 0x00, 0x00}, \
  {0x020E, 0x01, 0x00}, \
  {0x020F, 0x00, 0x00}, \
  /* Added Settings IQ */ \
  {0x94DC, 0x20, 0x00}, \
  {0x94DD, 0x20, 0x00}, \
  {0x94DE, 0x20, 0x00}, \
  {0x95DC, 0x20, 0x00}, \
  {0x95DD, 0x20, 0x00}, \
  {0x95DE, 0x20, 0x00}, \
  {0x7FB0, 0x00, 0x00}, \
  {0x9010, 0x3E, 0x00}, \
  /* Added Settings mode */ \
  {0x3030, 0x00, 0x00}, \
  {0x3032, 0x00, 0x00}, \
  {0x0220, 0x00, 0x00}, \
  /* MIPI Global Timing Settings*/ \
  {0x080A, 0x00, 0x00}, \
  {0x080B, 0x97, 0x00}, \
  {0x080C, 0x00, 0x00}, \
  {0x080D, 0x5F, 0x00}, \
  {0x080E, 0x00, 0x00}, \
  {0x080F, 0x97, 0x00}, \
  {0x0810, 0x00, 0x00}, \
  {0x0811, 0x57, 0x00}, \
  {0x0812, 0x00, 0x00}, \
  {0x0813, 0x57, 0x00}, \
  {0x0814, 0x00, 0x00}, \
  {0x0815, 0x5F, 0x00}, \
  {0x0816, 0x01, 0x00}, \
  {0x0817, 0x67, 0x00}, \
  {0x0818, 0x00, 0x00}, \
  {0x0819, 0x47, 0x00}, \
}

#define RES4_REG_ARRAY \
{ \
  /* Mode: 1400x784 90 fps*/ \
  /* Output Format Settings */ \
  {0x0112, 0x0A, 0x00}, \
  {0x0113, 0x0A, 0x00}, \
  {0x0114, 0x03, 0x00}, \
  /* Clock Settings */ \
  {0x0301, 0x05, 0x00}, \
  {0x0303, 0x02, 0x00}, \
  {0x0305, 0x04, 0x00}, \
  {0x0306, 0x00, 0x00}, \
  {0x0307, 0xD8, 0x00}, \
  {0x0309, 0x0A, 0x00}, \
  {0x030B, 0x01, 0x00}, \
  {0x030D, 0x02, 0x00}, \
  {0x030E, 0x00, 0x00}, \
  {0x030F, 0xD8, 0x00}, \
  {0x0310, 0x00, 0x00}, \
  {0x0820, 0x14, 0x00}, \
  {0x0821, 0x40, 0x00}, \
  {0x0822, 0x00, 0x00}, \
  {0x0823, 0x00, 0x00}, \
  /* Line Length Settings */ \
  {0x0342, 0x14, 0x00}, \
  {0x0343, 0xE8, 0x00}, \
  /* Frame Length Settings */ \
  {0x0340, 0x04, 0x00}, \
  {0x0341, 0x34, 0x00}, \
  /* ROI Settings */ \
  {0x0344, 0x00, 0x00}, \
  {0x0345, 0x00, 0x00}, \
  {0x0346, 0x01, 0x00}, \
  {0x0347, 0x80, 0x00}, \
  {0x0348, 0x10, 0x00}, \
  {0x0349, 0x6F, 0x00}, \
  {0x034A, 0x0A, 0x00}, \
  {0x034B, 0xAF, 0x00}, \
  /* Analog Image Size Settings */ \
  {0x0381, 0x01, 0x00}, \
  {0x0383, 0x01, 0x00}, \
  {0x0385, 0x03, 0x00}, \
  {0x0387, 0x03, 0x00}, \
  {0x0900, 0x01, 0x00}, \
  {0x0901, 0x12, 0x00}, \
  /* Digital Image Size Settings */ \
  {0x0401, 0x01, 0x00}, \
  {0x0404, 0x00, 0x00}, \
  {0x0405, 0x30, 0x00}, \
  {0x0408, 0x00, 0x00}, \
  {0x0409, 0x00, 0x00}, \
  {0x040A, 0x00, 0x00}, \
  {0x040B, 0x00, 0x00}, \
  {0x040C, 0x10, 0x00}, \
  {0x040D, 0x70, 0x00}, \
  {0x040E, 0x03, 0x00}, \
  {0x040F, 0x10, 0x00}, \
  {0x3038, 0x00, 0x00}, \
  {0x303A, 0x00, 0x00}, \
  {0x303B, 0x10, 0x00}, \
  {0x300D, 0x01, 0x00}, \
  /* Output Size Settings */ \
  {0x034C, 0x05, 0x00}, \
  {0x034D, 0x78, 0x00}, \
  {0x034E, 0x03, 0x00}, \
  {0x034F, 0x10, 0x00}, \
  /* Integration Time Settings */ \
  {0x0202, 0x04, 0x00}, \
  {0x0203, 0x2A, 0x00}, \
  /* Gain Settings */ \
  {0x0204, 0x00, 0x00}, \
  {0x0205, 0x00, 0x00}, \
  {0x020E, 0x01, 0x00}, \
  {0x020F, 0x00, 0x00}, \
  /* Added Settings IQ */ \
  {0x94DC, 0x20, 0x00}, \
  {0x94DD, 0x20, 0x00}, \
  {0x94DE, 0x20, 0x00}, \
  {0x95DC, 0x20, 0x00}, \
  {0x95DD, 0x20, 0x00}, \
  {0x95DE, 0x20, 0x00}, \
  {0x7FB0, 0x00, 0x00}, \
  {0x9010, 0x3E, 0x00}, \
  /* Added Settings mode */ \
  {0x3030, 0x00, 0x00}, \
  {0x3032, 0x00, 0x00}, \
  {0x0220, 0x00, 0x00}, \
  /* MIPI Global Timing Settings*/ \
  {0x080A, 0x00, 0x00}, \
  {0x080B, 0x97, 0x00}, \
  {0x080C, 0x00, 0x00}, \
  {0x080D, 0x5F, 0x00}, \
  {0x080E, 0x00, 0x00}, \
  {0x080F, 0x97, 0x00}, \
  {0x0810, 0x00, 0x00}, \
  {0x0811, 0x57, 0x00}, \
  {0x0812, 0x00, 0x00}, \
  {0x0813, 0x57, 0x00}, \
  {0x0814, 0x00, 0x00}, \
  {0x0815, 0x5F, 0x00}, \
  {0x0816, 0x01, 0x00}, \
  {0x0817, 0x67, 0x00}, \
  {0x0818, 0x00, 0x00}, \
  {0x0819, 0x47, 0x00}, \
}

#define RES5_REG_ARRAY \
{ \
  /* Mode: 1400x760 120 fps*/ \
  /* Output Format Settings */ \
  {0x0112, 0x0A, 0x00}, \
  {0x0113, 0x0A, 0x00}, \
  {0x0114, 0x03, 0x00}, \
  /* Clock Settings */ \
  {0x0301, 0x05, 0x00}, \
  {0x0303, 0x02, 0x00}, \
  {0x0305, 0x04, 0x00}, \
  {0x0306, 0x00, 0x00}, \
  {0x0307, 0xD8, 0x00}, \
  {0x0309, 0x0A, 0x00}, \
  {0x030B, 0x01, 0x00}, \
  {0x030D, 0x02, 0x00}, \
  {0x030E, 0x00, 0x00}, \
  {0x030F, 0xD8, 0x00}, \
  {0x0310, 0x00, 0x00}, \
  {0x0820, 0x14, 0x00}, \
  {0x0821, 0x40, 0x00}, \
  {0x0822, 0x00, 0x00}, \
  {0x0823, 0x00, 0x00}, \
  /* Line Length Settings */ \
  {0x0342, 0x14, 0x00}, \
  {0x0343, 0xE8, 0x00}, \
  /* Frame Length Settings */ \
  {0x0340, 0x03, 0x00}, \
  {0x0341, 0x24, 0x00}, \
  /* ROI Settings */ \
  {0x0344, 0x00, 0x00}, \
  {0x0345, 0x00, 0x00}, \
  {0x0346, 0x01, 0x00}, \
  {0x0347, 0xA0, 0x00}, \
  {0x0348, 0x10, 0x00}, \
  {0x0349, 0x6F, 0x00}, \
  {0x034A, 0x0A, 0x00}, \
  {0x034B, 0x87, 0x00}, \
  /* Analog Image Size Settings */ \
  {0x0381, 0x01, 0x00}, \
  {0x0383, 0x01, 0x00}, \
  {0x0385, 0x03, 0x00}, \
  {0x0387, 0x03, 0x00}, \
  {0x0900, 0x01, 0x00}, \
  {0x0901, 0x12, 0x00}, \
  /* Digital Image Size Settings */ \
  {0x0401, 0x01, 0x00}, \
  {0x0404, 0x00, 0x00}, \
  {0x0405, 0x30, 0x00}, \
  {0x0408, 0x00, 0x00}, \
  {0x0409, 0x00, 0x00}, \
  {0x040A, 0x00, 0x00}, \
  {0x040B, 0x00, 0x00}, \
  {0x040C, 0x10, 0x00}, \
  {0x040D, 0x70, 0x00}, \
  {0x040E, 0x02, 0x00}, \
  {0x040F, 0xF8, 0x00}, \
  {0x3038, 0x00, 0x00}, \
  {0x303A, 0x00, 0x00}, \
  {0x303B, 0x10, 0x00}, \
  {0x300D, 0x01, 0x00}, \
  /* Output Size Settings */ \
  {0x034C, 0x05, 0x00}, \
  {0x034D, 0x78, 0x00}, \
  {0x034E, 0x02, 0x00}, \
  {0x034F, 0xF8, 0x00}, \
  /* Integration Time Settings */ \
  {0x0202, 0x03, 0x00}, \
  {0x0203, 0x1A, 0x00}, \
  /* Gain Settings */ \
  {0x0204, 0x00, 0x00}, \
  {0x0205, 0x00, 0x00}, \
  {0x020E, 0x01, 0x00}, \
  {0x020F, 0x00, 0x00}, \
  /* Added Settings IQ */ \
  {0x94DC, 0x20, 0x00}, \
  {0x94DD, 0x20, 0x00}, \
  {0x94DE, 0x20, 0x00}, \
  {0x95DC, 0x20, 0x00}, \
  {0x95DD, 0x20, 0x00}, \
  {0x95DE, 0x20, 0x00}, \
  {0x7FB0, 0x00, 0x00}, \
  {0x9010, 0x3E, 0x00}, \
  /* Added Settings mode */ \
  {0x3030, 0x00, 0x00}, \
  {0x3032, 0x00, 0x00}, \
  {0x0220, 0x00, 0x00}, \
  /* MIPI Global Timing Settings*/ \
  {0x080A, 0x00, 0x00}, \
  {0x080B, 0x97, 0x00}, \
  {0x080C, 0x00, 0x00}, \
  {0x080D, 0x5F, 0x00}, \
  {0x080E, 0x00, 0x00}, \
  {0x080F, 0x97, 0x00}, \
  {0x0810, 0x00, 0x00}, \
  {0x0811, 0x57, 0x00}, \
  {0x0812, 0x00, 0x00}, \
  {0x0813, 0x57, 0x00}, \
  {0x0814, 0x00, 0x00}, \
  {0x0815, 0x5F, 0x00}, \
  {0x0816, 0x01, 0x00}, \
  {0x0817, 0x67, 0x00}, \
  {0x0818, 0x00, 0x00}, \
  {0x0819, 0x47, 0x00}, \
}

/* Sensor Handler */
static sensor_lib_t sensor_lib_ptr =
{
  .sensor_slave_info =
  {
    .sensor_name = SENSOR_MODEL,
    .slave_addr = 0x34,
    .i2c_freq_mode = SENSOR_I2C_MODE_FAST,
    .addr_type = CAMERA_I2C_WORD_ADDR,
    .sensor_id_info =
    {
      .sensor_id_reg_addr = 0x0016,
      .sensor_id = 0x0258,
    },
    .power_setting_array =
    {
      .power_setting_a =
      {
        {
          .seq_type = CAMERA_POW_SEQ_GPIO,
          .seq_val = CAMERA_GPIO_RESET, //Camera Reset Down
          .config_val = GPIO_OUT_LOW,
          .delay = 1,
        },
        {
          .seq_type = CAMERA_POW_SEQ_VREG,
          .seq_val = CAMERA_VDIG, // DVDD 1.2V L2
          .config_val = 0,
          .delay = 0,
        },
        {
          .seq_type = CAMERA_POW_SEQ_VREG,
          .seq_val = CAMERA_VIO, // DOVDD 1.8V L6
          .config_val = 0,
          .delay = 0,
        },
        {
          .seq_type = CAMERA_POW_SEQ_VREG,
          .seq_val = CAMERA_VAF, // af 2.8V L17
          .config_val = 0,
          .delay = 0,
        },
        {
          .seq_type = CAMERA_POW_SEQ_VREG,
          .seq_val = CAMERA_VANA, // AVDD L22
          .config_val = 0,
          .delay = 0,
        },
        {
          .seq_type = CAMERA_POW_SEQ_CLK,
          .seq_val = CAMERA_MCLK, //MCLK
          .config_val = 24000000,
          .delay = 1,
        },
        {
          .seq_type = CAMERA_POW_SEQ_GPIO,
          .seq_val = CAMERA_GPIO_RESET, //Camera Reset High
          .config_val = GPIO_OUT_HIGH,
          .delay = 12,
        },
      },
      .size = 7,
      .power_down_setting_a =
      {
        {
          .seq_type = CAMERA_POW_SEQ_GPIO,
          .seq_val = CAMERA_GPIO_RESET,
          .config_val = GPIO_OUT_LOW,
          .delay = 0,
        },
        {
          .seq_type = CAMERA_POW_SEQ_CLK,
          .seq_val = CAMERA_MCLK,
          .config_val = 0,
          .delay = 1,
        },
        {
          .seq_type = CAMERA_POW_SEQ_VREG,
          .seq_val = CAMERA_VANA, // AVDD L22
          .config_val = 0,
          .delay = 0,
        },
        {
          .seq_type = CAMERA_POW_SEQ_VREG,
          .seq_val = CAMERA_VAF,
          .config_val = 0,
          .delay = 0,
        },
        {
          .seq_type = CAMERA_POW_SEQ_VREG,
          .seq_val = CAMERA_VIO,
          .config_val = 0,
          .delay = 0,
        },
        {
          .seq_type = CAMERA_POW_SEQ_VREG,
          .seq_val = CAMERA_VDIG,
          .config_val = 0,
          .delay = 0,
        },
      },
      .size_down = 6,
    },
  },
  .sensor_output =
  {
    .output_format = SENSOR_BAYER,
    .connection_mode = SENSOR_MIPI_CSI,
    .raw_output = SENSOR_10_BIT_DIRECT,
#ifndef FLIP_MIRROR
    .filter_arrangement = SENSOR_RGGB,
#else
    .filter_arrangement = SENSOR_BGGR,
#endif
  },
  .output_reg_addr =
  {
    .x_output = 0x034C,
    .y_output = 0x034E,
    .line_length_pclk = 0x0342,
    .frame_length_lines = 0x0340,
  },
  .exp_gain_info =
  {
    .coarse_int_time_addr = 0x0202,
    .global_gain_addr = 0x0204,
    .vert_offset = IMX258_MS3_MAX_INTEGRATION_MARGIN,
  },
  .aec_info =
  {
    .min_gain = IMX258_MS3_MIN_GAIN,
    .max_gain = IMX258_MS3_MAX_GAIN,
    .max_analog_gain = IMX258_MS3_MAX_AGAIN,
    .max_linecount = 65525 - IMX258_MS3_MAX_INTEGRATION_MARGIN,
  },
  .sensor_num_frame_skip = 2,
  .sensor_num_HDR_frame_skip = 2,
  .sensor_max_pipeline_frame_delay = 2,
  .sensor_property =
  {
    .pix_size = 1.12, /* um */
    .sensing_method = SENSOR_SMETHOD_ONE_CHIP_COLOR_AREA_SENSOR,
    .crop_factor = 5.78,
  },
  .pixel_array_size_info =
  {
    .active_array_size =
    {
      .width = 4208,
      .height = 3120,
    },
    .left_dummy = 8,
    .right_dummy = 8,
    .top_dummy = 8,
    .bottom_dummy = 8,
  },
  .color_level_info =
  {
    .white_level = 1023,
    .r_pedestal = IMX258_MS3_DATA_PEDESTAL,
    .gr_pedestal = IMX258_MS3_DATA_PEDESTAL,
    .gb_pedestal = IMX258_MS3_DATA_PEDESTAL,
    .b_pedestal = IMX258_MS3_DATA_PEDESTAL,
  },
  .sensor_stream_info_array =
  {
    .sensor_stream_info =
    {
      {
        .vc_cfg_size = 2,
        .vc_cfg =
        {
          {
            .cid = 0,
            .dt = CSI_RAW10,
            .decode_format = CSI_DECODE_10BIT,
          },
          {
            .cid = 1,
            .dt = IMX258_MS3_CSI_PD_ISTATS,
            .decode_format = CSI_DECODE_8BIT,
          },
        },
        .pix_data_fmt =
        {
          SENSOR_BAYER,
          SENSOR_META,
        },
      },
      {
        .vc_cfg_size = 1,
        .vc_cfg =
        {
          {
            .cid = 2,
            .dt = CSI_EMBED_DATA,
            .decode_format = CSI_DECODE_8BIT,
          },
        },
        .pix_data_fmt =
        {
          SENSOR_META,
        },
      },
    },
    .size = 2,
  },
  .start_settings =
  {
    .reg_setting_a = START_REG_ARRAY,
    .addr_type = CAMERA_I2C_WORD_ADDR,
    .data_type = CAMERA_I2C_BYTE_DATA,
    .delay = 0,
  },
  .stop_settings =
  {
    .reg_setting_a = STOP_REG_ARRAY,
    .addr_type = CAMERA_I2C_WORD_ADDR,
    .data_type = CAMERA_I2C_BYTE_DATA,
    .delay = 0,
  },
  .groupon_settings =
  {
    .reg_setting_a = GROUPON_REG_ARRAY,
    .addr_type = CAMERA_I2C_WORD_ADDR,
    .data_type = CAMERA_I2C_BYTE_DATA,
    .delay = 0,
  },
  .groupoff_settings =
  {
    .reg_setting_a = GROUPOFF_REG_ARRAY,
    .addr_type = CAMERA_I2C_WORD_ADDR,
    .data_type = CAMERA_I2C_BYTE_DATA,
    .delay = 0,
  },
  .embedded_data_enable_settings =
  {
    .reg_setting_a = {},
    .addr_type = 0,
    .data_type = 0,
    .delay = 0,
  },
  .embedded_data_disable_settings =
  {
    .reg_setting_a = {},
    .addr_type = 0,
    .data_type = 0,
    .delay = 0,
  },
  .test_pattern_info =
  {
    .test_pattern_settings =
    {
      {
        .mode = SENSOR_TEST_PATTERN_OFF,
        .settings =
        {
          .reg_setting_a =
          {
            {0x0600, 0x0000, 0x00},
          },
          .addr_type = CAMERA_I2C_WORD_ADDR,
          .data_type = CAMERA_I2C_WORD_DATA,
          .delay = 0,
        }
      },
      {
        .mode = SENSOR_TEST_PATTERN_SOLID_COLOR,
        .settings =
        {
          .reg_setting_a =
          {
            {0x0600, 0x0001, 0x00},
          },
          .addr_type = CAMERA_I2C_WORD_ADDR,
          .data_type = CAMERA_I2C_WORD_DATA,
          .delay = 0,
        },
      },
      {
        .mode = SENSOR_TEST_PATTERN_COLOR_BARS,
        .settings =
        {
          .reg_setting_a =
          {
            {0x0600, 0x0002, 0x00},
          },
          .addr_type = CAMERA_I2C_WORD_ADDR,
          .data_type = CAMERA_I2C_WORD_DATA,
          .delay = 0,
        },
      },
      {
        .mode = SENSOR_TEST_PATTERN_COLOR_BARS_FADE_TO_GRAY,
        .settings =
        {
          .reg_setting_a =
          {
            {0x0600, 0x0003, 0x00},
          },
          .addr_type = CAMERA_I2C_WORD_ADDR,
          .data_type = CAMERA_I2C_WORD_DATA,
          .delay = 0,
        },
      },
      {
        .mode = SENSOR_TEST_PATTERN_PN9,
        .settings =
        {
          .reg_setting_a =
          {
            {0x0600, 0x0004, 0x00},
          },
          .addr_type = CAMERA_I2C_WORD_ADDR,
          .data_type = CAMERA_I2C_WORD_DATA,
          .delay = 0,
        },
      },
    },
    .size = 5,
    .solid_mode_addr =
    {
      .r_addr = 0x0602,
      .gr_addr = 0x0604,
      .gb_addr = 0x0606,
      .b_addr = 0x0608,
    },
  },
  .init_settings_array =
  {
    .reg_settings =
    {
      {
        .reg_setting_a = INIT0_REG_ARRAY,
        .addr_type = CAMERA_I2C_WORD_ADDR,
        .data_type = CAMERA_I2C_BYTE_DATA,
        .delay = 0,
      },
      {
        .reg_setting_a = FLIP_MIRROR_SETTING,
        .addr_type = CAMERA_I2C_WORD_ADDR,
        .data_type = CAMERA_I2C_BYTE_DATA,
        .delay = 0,
      },
    },
    .size = 2,
  },
  .res_settings_array =
  {
    .reg_settings =
    {
      /* Res 0 */
      {
        .reg_setting_a = RES0_REG_ARRAY,
        .addr_type = CAMERA_I2C_WORD_ADDR,
        .data_type = CAMERA_I2C_BYTE_DATA,
        .delay = 0,
      },
      /* Res 1 */
      {
        .reg_setting_a = RES1_REG_ARRAY,
        .addr_type = CAMERA_I2C_WORD_ADDR,
        .data_type = CAMERA_I2C_BYTE_DATA,
        .delay = 0,
      },
      /* Res 2 */
      {
        .reg_setting_a = RES2_REG_ARRAY,
        .addr_type = CAMERA_I2C_WORD_ADDR,
        .data_type = CAMERA_I2C_BYTE_DATA,
        .delay = 0,
      },
      /* Res 3 */
      {
        .reg_setting_a = RES3_REG_ARRAY,
        .addr_type = CAMERA_I2C_WORD_ADDR,
        .data_type = CAMERA_I2C_BYTE_DATA,
        .delay = 0,
      },
      /* Res 4 */
      {
        .reg_setting_a = RES4_REG_ARRAY,
        .addr_type = CAMERA_I2C_WORD_ADDR,
        .data_type = CAMERA_I2C_BYTE_DATA,
        .delay = 0,
      },
      /* Res 5 */
      {
        .reg_setting_a = RES5_REG_ARRAY,
        .addr_type = CAMERA_I2C_WORD_ADDR,
        .data_type = CAMERA_I2C_BYTE_DATA,
        .delay = 0,
      },
      },
    .size = 6,
  },
  .out_info_array =
  {
    .out_info =
    {
      /* Res 0 */
      {
        .x_output = 4208,
        .y_output = 3120,
        .line_length_pclk = 5352,
        .frame_length_lines = 3224,
        .vt_pixel_clk = 518400000,
        .op_pixel_clk = 518400000,
        .binning_factor = 1,
        .min_fps = 7.500,
        .max_fps = 30.04,
        .mode = SENSOR_DEFAULT_MODE,
        .offset_x = 0,
        .offset_y = 0,
        .scale_factor = 1.000,
        .is_pdaf_supported = 0,
      },
      /* Res 1 */
      {
        .x_output = 4208,
        .y_output = 2368,
        .line_length_pclk = 5352,
        .frame_length_lines = 2400,
        .vt_pixel_clk = 386400000,
        .op_pixel_clk = 386400000,
        .binning_factor = 1,
        .min_fps = 7.500,
        .max_fps = 30.04,
        .mode = SENSOR_DEFAULT_MODE,
        .offset_x = 0,
        .offset_y = 368,
        .scale_factor = 1.000,
        .is_pdaf_supported = 0,
      },
      /* Res 2 */
      {
        .x_output = 2100,
        .y_output = 1560,
        .line_length_pclk = 5352,
        .frame_length_lines = 3184,
        .vt_pixel_clk = 518400000,
        .op_pixel_clk = 518400000,
        .binning_factor = 2,
        .min_fps = 7.500,
        .max_fps = 30.42,
        .mode = SENSOR_DEFAULT_MODE,
        .offset_x = 0,
        .offset_y = 0,
        .scale_factor = 1.000,
        .is_pdaf_supported = 0,
      },
      /* Res 3*/
      {
        .x_output = 2100,
        .y_output = 1176,
        .line_length_pclk = 5352,
        .frame_length_lines = 1612,
        .vt_pixel_clk = 518400000,
        .op_pixel_clk = 518400000,
        .binning_factor = 2,
        .min_fps = 7.500,
        .max_fps = 60.08,
        .mode = SENSOR_HFR_MODE,
        .offset_x = 0,
        .offset_y = 0,
        .scale_factor = 1.000,
        .is_pdaf_supported = 0,
      },
      /* Res 5 */
      {
        .x_output = 1400,
        .y_output = 784,
        .line_length_pclk = 5352,
        .frame_length_lines = 1076,
        .vt_pixel_clk = 518400000,
        .op_pixel_clk = 518400000,
        .binning_factor = 2,
        .min_fps = 7.500,
        .max_fps = 90.18,
        .mode = SENSOR_HFR_MODE,
        .offset_x = 0,
        .offset_y = 0,
        .scale_factor = 1.000,
        .is_pdaf_supported = 0,
      },
      /* Res 6 */
      {
        .x_output = 1400,
        .y_output = 760,
        .line_length_pclk = 5352,
        .frame_length_lines = 804,
        .vt_pixel_clk = 518400000,
        .op_pixel_clk = 518400000,
        .binning_factor = 4,
        .min_fps = 7.500,
        .max_fps = 120.47,
        .mode = SENSOR_HFR_MODE,
        .offset_x = 0,
        .offset_y = 0,
        .scale_factor = 1.000,
        .is_pdaf_supported = 0,
      },
    },
    .size = 6,
  },
  .csi_params =
  {
    .lane_cnt = 4,
    .settle_cnt = 0xB,
    .is_csi_3phase = 0,
  },
  .csid_lut_params_array =
  {
    .lut_params =
    {
      /* Res 0 */
      {
        .num_cid = 2,
        .vc_cfg_a =
        {
          {
            .cid = 0,
            .dt = CSI_RAW10,
            .decode_format = CSI_DECODE_10BIT
          },
          {
            .cid = 1,
            .dt = IMX258_MS3_CSI_PD_ISTATS,
            .decode_format = CSI_DECODE_8BIT
          },
        },
      },
      /* Res 1 */
      {
        .num_cid = 2,
        .vc_cfg_a =
        {
          {
            .cid = 0,
            .dt = CSI_RAW10,
            .decode_format = CSI_DECODE_10BIT
          },
          {
            .cid = 1,
            .dt = IMX258_MS3_CSI_PD_ISTATS,
            .decode_format = CSI_DECODE_8BIT
          },
        },
      },
      /* Res 2 */
      {
        .num_cid = 2,
        .vc_cfg_a =
        {
          {
            .cid = 0,
            .dt = CSI_RAW10,
            .decode_format = CSI_DECODE_10BIT
          },
          {
            .cid = 2,
            .dt = CSI_EMBED_DATA,
            .decode_format = CSI_DECODE_8BIT
          },
        },
      },
      /* Res 3 */
      {
        .num_cid = 2,
        .vc_cfg_a =
        {
          {
            .cid = 0,
            .dt = CSI_RAW10,
            .decode_format = CSI_DECODE_10BIT
          },
          {
            .cid = 2,
            .dt = CSI_EMBED_DATA,
            .decode_format = CSI_DECODE_8BIT
          },
        },
      },
      /* Res 4 */
      {
        .num_cid = 2,
        .vc_cfg_a =
        {
          {
            .cid = 0,
            .dt = CSI_RAW10,
            .decode_format = CSI_DECODE_10BIT
          },
          {
            .cid = 2,
            .dt = CSI_EMBED_DATA,
            .decode_format = CSI_DECODE_8BIT
          },
        },
      },
      /* Res 5 */
      {
        .num_cid = 2,
        .vc_cfg_a =
        {
          {
            .cid = 0,
            .dt = CSI_RAW10,
            .decode_format = CSI_DECODE_10BIT
          },
          {
            .cid = 2,
            .dt = CSI_EMBED_DATA,
            .decode_format = CSI_DECODE_8BIT
          },
        },
      },
    },
    .size = 6,
  },
  .crop_params_array =
  {
    .crop_params =
    {
      /* Res 0 */
      {
        .top_crop = 0,
        .bottom_crop = 0,
        .left_crop = 0,
        .right_crop = 0,
      },
      /* Res 1 */
      {
        .top_crop = 0,
        .bottom_crop = 0,
        .left_crop = 0,
        .right_crop = 0,
      },
      /* Res 2 */
      {
        .top_crop = 0,
        .bottom_crop = 0,
        .left_crop = 0,
        .right_crop = 0,
      },
      /* Res 3 */
      {
        .top_crop = 0,
        .bottom_crop = 0,
        .left_crop = 0,
        .right_crop = 0,
      },
      /* Res 4 */
      {
        .top_crop = 0,
        .bottom_crop = 0,
        .left_crop = 0,
        .right_crop = 0,
      },
      /* Res 5 */
      {
        .top_crop = 0,
        .bottom_crop = 0,
        .left_crop = 0,
        .right_crop = 0,
      },
    },
    .size = 6,
  },
  .exposure_func_table =
  {
    .sensor_calculate_exposure = sensor_calculate_exposure,
    .sensor_fill_exposure_array = sensor_fill_exposure_array,
  },
  .meta_data_out_info_array =
  {
    .meta_data_out_info =
    {
      {
        /* set the meta half size which it should be to overcome the isp bug */
        .width = 80,
        .height = 1920,
        .stats_type = PD_STATS,
        .dt = IMX258_MS3_CSI_PD_ISTATS,
      },
    },
    .size = 1,
  },
  .sensor_capability = 0,
  .awb_func_table =
  {
    .sensor_fill_awb_array = 0,
    .awb_table_size = 0,
  },
  .parse_RDI_stats =
  {
    .parse_VHDR_stats = NULL,
  },
  .rolloff_config =
  {
    .enable = FALSE,
    .full_size_info =
    {
      .full_size_width = 0,
      .full_size_height = 0,
      .full_size_left_crop = 0,
      .full_size_top_crop = 0,
    },
  },
  .adc_readout_time = 0,
  .sensor_num_fast_aec_frame_skip = 0,
  .noise_coeff = {
    .gradient_S = 3.738032e-06,
    .offset_S = 3.651935e-04,
    .gradient_O = 4.499952e-07,
    .offset_O = -2.968624e-04,
  },
  .pdaf_config = {
#ifndef FLIP_MIRROR
    #include "imx258_ms3_pdaf.h"
#else
    #include "imx258_ms3_pdaf_flip_mirror.h"
#endif
  },
};

#endif /* __IMX258_MS3_LIB_H__ */
