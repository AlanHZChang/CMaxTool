...
static unsigned int imx258_ms3_real_to_register_gain(float real_gain)
{
    unsigned int reg_gain = 0;

    if (real_gain < IMX258_MS3_MIN_AGAIN)
    {
        real_gain = IMX258_MS3_MIN_AGAIN;
        SHIGH("IMX258_MS3 Requested gain(%f) is lower than the sensor minimum(%d)",
            real_gain, IMX258_MS3_MIN_AGAIN);
    }
    else if (real_gain > IMX258_MS3_MAX_AGAIN)
    {
        real_gain = IMX258_MS3_MAX_AGAIN;
    }
...
/**
 * FUNCTION: imx258_ms3_register_to_real_gain
 *
 * DESCRIPTION: Calcuate sensor real gain value
 **/
static float imx258_ms3_register_to_real_gain(unsigned int reg_gain)
{
    float gain;

    if(reg_gain > IMX258_MS3_MAX_AGAIN_REG_VAL)
        reg_gain = IMX258_MS3_MAX_AGAIN_REG_VAL;
...
/**
 * FUNCTION: imx258_ms3_digital_gain_calc
 *
 * DESCRIPTION: Calcuate the sensor digital gain
 **/
static unsigned int imx258_ms3_digital_gain_calc(
 float real_gain, float sensor_real_gain)
{
    unsigned int reg_dig_gain = IMX258_MS3_MIN_DGAIN_REG_VAL;
    float real_dig_gain = IMX258_MS3_MIN_DGAIN;

    if(real_gain > IMX258_MS3_MAX_AGAIN)
    {
        real_dig_gain = real_gain / sensor_real_gain;
    }
    else
    {
        real_dig_gain = IMX258_MS3_MIN_DGAIN;
    }

    if(real_dig_gain > IMX258_MS3_MAX_DGAIN)
    {
        real_dig_gain = IMX258_MS3_MAX_DGAIN;
    }
  ...
  __attribute__((unused))float s_real_gain)
{
    ...
    exp_info->reg_gain = imx258_ms3_real_to_register_gain(real_gain);
    exp_info->sensor_real_gain =
      imx258_ms3_register_to_real_gain(exp_info->reg_gain);
    exp_info->sensor_digital_gain =
      imx258_ms3_digital_gain_calc(real_gain, exp_info->sensor_real_gain);
    exp_info->sensor_real_dig_gain =
      (float)exp_info->sensor_digital_gain / IMX258_MS3_MAX_DGAIN_DECIMATOR;
...
/**
 * FUNCTION: imx258_ms3_fill_exposure_array
 *
  ...
  __attribute__((unused)) int is_hdr_enabled)
{
    ...
    reg_setting->reg_setting[reg_count].reg_addr = IMX258_MS3_DIG_GAIN_GR_ADDR;
    reg_setting->reg_setting[reg_count].reg_data = (digital_gain & 0x0000FF00) >> 8;
    reg_setting->reg_setting[reg_count].delay = 0;
    reg_count = reg_count + 1;

    reg_setting->reg_setting[reg_count].reg_addr = IMX258_MS3_DIG_GAIN_GR_ADDR + 1;
    reg_setting->reg_setting[reg_count].reg_data = (digital_gain & 0x000000FF);
    reg_setting->reg_setting[reg_count].delay = 0;
    reg_count = reg_count + 1;

    reg_setting->reg_setting[reg_count].reg_addr = IMX258_MS3_DIG_GAIN_R_ADDR;
    reg_setting->reg_setting[reg_count].reg_data = (digital_gain & 0x0000FF00) >> 8;
    reg_setting->reg_setting[reg_count].delay = 0;
    reg_count = reg_count + 1;

    reg_setting->reg_setting[reg_count].reg_addr = IMX258_MS3_DIG_GAIN_R_ADDR + 1;
    reg_setting->reg_setting[reg_count].reg_data = (digital_gain & 0x000000FF);
    reg_setting->reg_setting[reg_count].delay = 0;
    reg_count = reg_count + 1;

    reg_setting->reg_setting[reg_count].reg_addr = IMX258_MS3_DIG_GAIN_B_ADDR;
    reg_setting->reg_setting[reg_count].reg_data = (digital_gain & 0x0000FF00) >> 8;
    reg_setting->reg_setting[reg_count].delay = 0;
    reg_count = reg_count + 1;

    reg_setting->reg_setting[reg_count].reg_addr = IMX258_MS3_DIG_GAIN_B_ADDR + 1;
    reg_setting->reg_setting[reg_count].reg_data = (digital_gain & 0x000000FF);
    reg_setting->reg_setting[reg_count].delay = 0;
    reg_count = reg_count + 1;

    reg_setting->reg_setting[reg_count].reg_addr = IMX258_MS3_DIG_GAIN_GB_ADDR;
    reg_setting->reg_setting[reg_count].reg_data = (digital_gain & 0x0000FF00) >> 8;
    reg_setting->reg_setting[reg_count].delay = 0;
    reg_count = reg_count + 1;

    reg_setting->reg_setting[reg_count].reg_addr = IMX258_MS3_DIG_GAIN_GB_ADDR + 1;
    reg_setting->reg_setting[reg_count].reg_data = (digital_gain & 0x000000FF);
    reg_setting->reg_setting[reg_count].delay = 0;
    reg_count = reg_count + 1;
